...
for (i in farms){
  ...
  #######ALL CALVES (INCLUDING PURCHASED)
...
'))
  ...
  bornonfarm <- as.data.table(dbGetQuery(con, '
                                         
    SELECT
    
    mv.animalId as calfid,
    mv.type as bornonfarm
...
'))
  ...
  collated <- merge(calves, bornonfarm, by = "calfid", all.x = TRUE)
  
  collated$bornonfarm[which(is.na(collated$bornonfarm))] <- 0
  
  collated <- merge(collated, calves_dob, by = "calfid", all.x = TRUE)
  ...
  collated <- collated[,c(1,3,4,5,6:90,2,101,92:100,91,102)]
  ...
  write.csv(data_indi_wide, paste("y:/ian/johnesthresholds/johnesproper/data/30CowScreens/TVURaiderOutput/",FARM,"_RSQL_IncPurchased.csv",sep = ""), row.names = FALSE)
