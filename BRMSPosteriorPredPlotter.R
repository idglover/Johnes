...
jplotbf <- function(dt, id, tn, npd, ivl){
  ...
  post_draws_pos <- posterior_predict(brms_mod_short_12m_lognorm_pos_age_yield_scc_mtnc_dimind, 
                                      newdata = tempdata,
                                      ndraws = n_post_draws)
  ...
  post_draws_neg <- posterior_predict(brms_mod_short_12m_lognorm_neg_yield_scc_mtnc_dimind,
                                      newdata = tempdata,
                                      ndraws = n_post_draws)
...
jplotbf(data_brms, "UK342393100949", 1, 1000, 3)
